...
const TaskSidebar = props => {
  ...
  const [title, setTitle] = useState(''),
    [assignee, setAssignee] = useState({ value: 'ananda college', label: 'Ananda College'}),
  ...
  const handleSidebarClosed = () => {
    ...
    setDesc('')
    setTitle('')
    setAssignee({ value: 'ananda college', label: 'Ananda College'})
    setCompleted(false)
    setImportant(false)
    setDueDate(new Date())
    dispatch(selectTask({}))
